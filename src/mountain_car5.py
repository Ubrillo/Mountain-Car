...
global success_count, fail_count, timeout_count
success_count = 0
fail_count = 0
timeout_count = 0
...
def train_q_learning(runs=5, episodes=10):
    ...
    for run in range(runs):
        episode_data = []
        global epsilon, Q, action_space, position, fail_count, success_count, timeout_count
        epsilon = 0.1
        Q = np.zeros((n_bins, len(action_space)))
        print(f"\n=== Run {run+1}/{runs} ===")
        ...
        for episode in range(episodes):
            ...
            print(f"Episode {episode+1} - Steps: {steps} - Outcome: {outcome} - Epsilon: {epsilon:.3f} -acition: {action_space} - success: {success_count} failure: {fail_count} timeout: {timeout_count}")
...
plt.figure(figsize=(10, 5))
data = [success_count, fail_count, timeout_count]
labels = ['Success', 'Fail', 'Timeout']
